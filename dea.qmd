...
- 径向模型（Charnes 等，1978；Banker 等，1984）：`basic_DEA()` 计算标准效率得分，`super_DEA()` 进一步区分有效单元，允许超效率值大于 1；
- SBM 模型（Tone，2001）：`basic_SBM()` 和 `super_SBM()` 采用基于松弛变量的测度方法，弥补径向模型忽略非径向松弛的不足。
